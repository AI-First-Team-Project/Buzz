...
API/Kafka 연동 전 발표/개발 확인을 위해 홈 하단에 작은 `개발 데모` 버튼을 남겨두었습니다.
...
- 홈 `마지막 분석`은 더 이상 고정 12초가 아니라 UI 데모에서 10~30초 랜덤 간격의 Kafka 입력 시뮬레이션마다 갱신
- 실제 연동 시 FastAPI/Kafka Consumer 분석 결과 수신 시 현재 시각을 기록하는 방식으로 교체
