...
export function Worksites() {
  ...
  return <div className={styles.page}>
    ...
    <section className={styles.tableWrap}><table><thead><tr><th>사업장</th><th>위치</th><th>상태</th><th>개폐기</th><th>최근 감지</th><th>관리</th></tr></thead><tbody>{filteredSites.map((site)=><tr key={site.id}>
      ...
      <td><span className={`${styles.status} ${site.status==='danger'?styles.danger:styles.normal}`}><i/>{site.status==='danger'?'위험':'정상'}</span></td><td><span className={`${styles.door} ${site.door==='closed'?styles.closed:''}`}><DoorIcon size={14}/>{site.door==='closed'?'닫힘':'열림'}</span></td><td className={styles.time}>{formatTime(site.lastAnalyzedAt)}</td><td><button className={styles.manageButton} type="button" onClick={()=>navigate(`/?site=${site.id}`)}>관리하기</button></td>
