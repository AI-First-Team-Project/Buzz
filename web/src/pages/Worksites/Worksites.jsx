...
import './WorksitesResponsive.css';
...
export function Worksites() {
  ...
  return <div className={styles.page}>
    <header className={`${styles.header} worksites-responsive-header`}><div><h1>사업장 관리</h1><p>등록된 사업장 정보를 확인하고 운영 상태를 관리할 수 있습니다.</p></div><button className={styles.addButton} type="button" onClick={()=>setFormOpen(true)}>＋ 사업장 등록</button></header>
    <section className={styles.toolbar} aria-label="사업장 필터와 검색"><div className={`${styles.filters} worksites-responsive-filters`}><button type="button" className={filter==='all'?styles.filterActive:styles.filter} onClick={()=>setFilter('all')}>전체 {sites.length}</button><button type="button" className={filter==='normal'?styles.normalActive:styles.filter} onClick={()=>setFilter('normal')}>정상 {normalCount}</button><button type="button" className={filter==='danger'?styles.dangerActive:styles.filter} onClick={()=>setFilter('danger')}>위험 {dangerCount}</button></div><label className={styles.searchBox}><SearchIcon size={16}/><input value={query} onChange={(event)=>setQuery(event.target.value)} placeholder="사업명 또는 위치 검색"/></label></section>
    <section className={`${styles.tableWrap} worksites-mobile-cards`}><table><thead><tr><th>사업장</th><th>위치</th><th>상태</th><th>개폐기</th><th>최근 감지</th><th>관리</th></tr></thead><tbody>{filteredSites.map((site)=><tr key={site.id}>
      ...
      <td><span className={`${styles.status} ${site.status==='danger'?styles.danger:styles.normal}`}><i/>{site.status==='danger'?'위험':'정상'}</span></td><td><span className={`${styles.door} ${site.door==='closed'?styles.closed:''}`}><DoorIcon size={14}/>{site.door==='closed'?'닫힘':'열림'}</span></td><td className={styles.time}>{formatTime(site.lastAnalyzedAt)}</td><td><button className={styles.manageButton} type="button" onClick={()=>navigate(`/?site=${String(site.id).replace('site-','')}`)}>관리하기</button></td>
