...
function SiteCard({site, active, onClick, sim}) {
  ...
  return <button className={`ed-site-card ${active?'active':''} ${site.status==='danger'?'danger':''}`} onClick={onClick}>
    ...
    <div className="ed-simline"><span className={`ed-dot ${error?'error':sim?.state==='RUNNING'?'live':''}`}/><span>{site.workerStatus==='DEGRADED'?'분석 지연 · 마지막 판정 유지':sim?.current_file||'음원 대기'}</span>{Number.isInteger(sim?.chunk_index)&&<b>#{sim.chunk_index+1}</b>}</div>
  </button>;
}
function Detail({site, sim}) {
  ...
  return <section className="ed-detail">
    <div className="ed-detail-top"><div><p>REAL-TIME MONITORING</p><h2>{site.name}</h2><span>{site.workerStatus==='DEGRADED'?'분석 지연 · 마지막 판정 유지':sim?.current_file||'현재 입력 파일 없음'} {Number.isInteger(sim?.chunk_index)?`· chunk ${sim.chunk_index+1} · ${sim.chunk_start}~${sim.chunk_end}초`:''}</span><span>마지막 분석: {site.lastAnalyzedAt?new Date(site.lastAnalyzedAt).toLocaleString('ko-KR',{hour12:false}):'없음'}</span></div><div className="ed-live-result"><StatusPill site={site}/><strong>{site.aiLabel==='wasp'?'말벌':'말벌 아님'}</strong><b>{site.aiConfidence}%</b></div></div>
...
export function EnterpriseDashboard(){
 const {sites}=useMonitoring(); const [params,setParams]=useSearchParams(); const selected=params.get('site')||'all'; const [sim,setSim]=useState({enabled:true,sites:{}}); const [simError,setSimError]=useState(false);
 useEffect(()=>{let dead=false; const load=()=>fetchSimulatorStatus().then(v=>{if(!dead){setSim(v);setSimError(false)}}).catch(()=>{if(!dead)setSimError(true)});load();const t=setInterval(load,1500);return()=>{dead=true;clearInterval(t)}},[]);
 ...
 const toggle=async()=>{try{const next=sim.enabled?await stopSimulator():await startSimulator();setSim(next);setSimError(false)}catch{setSimError(true)}};
 return <div className="ed-page">
  ...
  {simError&&<div className="ed-error" role="alert">시뮬레이터 상태를 확인하지 못했습니다. 마지막으로 받은 정보를 표시합니다.</div>}
