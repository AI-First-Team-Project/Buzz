  ...
  return (
    <div className="buzz-commercial-page">
      ...
      <main className="buzz-commercial-content">
        <div className="buzz-stat-row">
          <span className="buzz-stat-pill normal">
            정상 <b>{sites.filter((s) => s.status !== "danger").length}</b>
          </span>
          <span className="buzz-stat-pill danger">
            위험 <b>{sites.filter((s) => s.status === "danger").length}</b>
          </span>
          <span className="buzz-stat-pill time">
        ...
        <p className="buzz-kicker" style={{ margin: "2px 0 6px" }}>사업장별 현황</p>
        <div className="buzz-mini-site-row">
          {sites.map((site) => (
            <button
              key={site.id}
              className={`buzz-mini-site-card ${site.status === "danger" ? "danger" : ""} ${
                site.id === selectedSiteId ? "selected" : ""
              }`}
              onClick={() => switchSite(site.id)}
            >
              <div className="buzz-mini-site-card-top">
                <b>{site.name}</b>
                <span className={`buzz-status-chip ${site.status === "danger" ? "danger" : ""}`}>
                  {site.status === "danger" ? "위험" : "정상"}
                </span>
              </div>
              <small>최근 AI 판정 {site.status === "danger" ? "말벌" : "말벌 아님"}</small>
            </button>
          ))}
        </div>
